...
//        ));
	...
	function people() {        
		...
		//pagination config
		$additional_segment = 'user/people';
		$per_page = 50;
    ...
    function render_datatable() {
        ...
        foreach ($data_rows as $result) {
            ...
            /* $action_html.='&nbsp;';
              $action_html.= anchor(base_url($this->router->directory.'user/delete/' . $result['id']), 'Delete', array(
        ...
        if ($this->input->post('form_action') == 'forgot_password') {
            if ($this->validate_forgot_password_form() == true) {
                ...
                if ($result) {
                    ...
                    $html.= anchor(base_url($this->router->directory.'user/reset_password/' . md5($password_reset_key)), NULL);
                    $html.='</p>';
                    $html.='</div>';
                    $html.='</div>';
